...
function setupSnapObservers(){
  ...
  // Glass boxes — re-trigger every time overview section enters
  const overview = document.getElementById('section-overview');
  const aboutSection = document.getElementById('section-about');
  [overview, aboutSection].forEach(function(sec){
    if(!sec) return;
  ...
  // Stats count-up — re-trigger every time stats section enters
  ...
  // Edu cards — re-trigger
  const learnSection = document.getElementById('section-learn');
  if(learnSection){
    const learnObs = new IntersectionObserver(function(entries){
      entries.forEach(function(e){
        const cards = learnSection.querySelectorAll('.edu-card, .flip-card, .quiz-teaser');
        if(e.isIntersecting){
          cards.forEach(function(c){ c.classList.remove('card-in'); });
          setTimeout(function(){
            cards.forEach(function(c, i){
              setTimeout(function(){ c.classList.add('card-in'); }, i * 80);
            });
          }, 60);
        } else {
          cards.forEach(function(c){ c.classList.remove('card-in'); });
  ...
  if(visualSection){
    const visObs = new IntersectionObserver(function(entries){
      entries.forEach(function(e){
        const cards = visualSection.querySelectorAll('.visual-card, .chart-bar');
        if(e.isIntersecting){
          cards.forEach(function(c){ c.classList.remove('vis-in'); });
          setTimeout(function(){
            cards.forEach(function(c, i){
              setTimeout(function(){ c.classList.add('vis-in'); }, i * 100);
            });
          }, 60);
          animateChartBars();
        } else {
...
function buildStats(){
  const groups = [
    [['urls','URLs Scanned'],['passwords','Passwords Checked'],['emails','Emails Verified'],['files','Files Scanned'],['qr','QR Codes'],['headers','Headers Analyzed']],
    [['threats','Threats Detected'],['phishing','Phishing Caught'],['weakpw','Weak Passwords'],['suspicious','Suspicious Emails'],['malwarefound','Malware Found'],['quizzes','Quizzes Done']],
    [['games','Games Played'],['totalweek','Scans This Week'],['protected','People Protected'],['prevented','Attacks Prevented'],['awareness','Awareness Score'],['urls','Total Checks']]
  ];
  groups.forEach(function(g, gi){
    const el = document.getElementById('stats-grid-' + (gi+1));
    if(!el) return;
    el.innerHTML = g.map(function(pair){
      return '<div class="stat-card"><div class="stat-num" data-target="' + STATS_DEFAULT[pair[0]] + '">0</div><div class="stat-lbl">' + pair[1] + '</div></div>';
    }).join('');
  });
}
...
function buildEdu(){
  ...
  if(!el) return;
  const tips = [
    {tag:'Tip of the Day', title:'Enable 2FA on every account — a stolen password alone won\'t be enough to break in.', icon:'🔐'},
    {tag:'Did You Know?', title:'A hacker attacks a computer every 39 seconds on average, somewhere in the world.', icon:'⚡'},
    {tag:'Threat of the Week', title:'QR code phishing ("quishing") is rising — always verify before scanning unknown codes.', icon:'🎣'},
    {tag:'Quick Fact', title:'95% of cybersecurity breaches are caused by human error, not technical failures.', icon:'🧠'},
    {tag:'Pro Tip', title:'Never reuse passwords. One breach exposes every account that shares that password.', icon:'🔑'},
    {tag:'Did You Know?', title:'The average cost of a data breach in 2024 was $4.88 million — an all-time high.', icon:'💸'},
  ];

  let html = tips.map(function(c, i){
    return '<div class="edu-card card-in-ready" style="animation-delay:' + (i*0.08) + 's">' +
  ...
  html += '<div class="quiz-teaser card-in-ready">' +
    '<div class="qt-label">Test Your Knowledge</div>' +
    '<div class="qt-title">How cyber-aware are you?</div>' +
    '<div class="qt-desc">Take the Cyber IQ Test and find out your security rating in under 3 minutes.</div>' +
    '<div class="nexus-btn qt-btn" data-go="intro-cyber_iq">TAKE THE TEST →</div>' +
    '</div>';

  el.innerHTML = html;

  // Flip card next-tip cycling
  let tipIdx = 0;
  setInterval(function(){
    const cards = document.querySelectorAll('.edu-card');
    if(!cards.length) return;
    tipIdx = (tipIdx + 1) % cards.length;
    cards.forEach(function(c, i){ c.style.opacity = i === tipIdx ? '1' : '0.7'; });
  }, 3000);
}
...
function buildVisuals(){
  ...
  if(map){
    const origins = [
      {country:'Russia', num:'1,204', color:'var(--c-red)', pct:92},
      {country:'China', num:'980', color:'var(--c-orange)', pct:75},
      {country:'USA', num:'645', color:'var(--gold)', pct:49},
      {country:'North Korea', num:'520', color:'var(--c-purple)', pct:40},
      {country:'Brazil', num:'412', color:'var(--teal-bright)', pct:32},
      {country:'India', num:'389', color:'var(--teal)', pct:30},
      {country:'Iran', num:'312', color:'var(--c-crimson)', pct:24},
      {country:'Ukraine', num:'287', color:'var(--c-deeporange)', pct:22},
    ];
    map.innerHTML = origins.map(function(o){
      return '<div class="map-row">' +
        '<span class="map-country">' + o.country + '</span>' +
        '<div class="map-bar-wrap"><div class="map-bar chart-bar" data-pct="' + o.pct + '" style="background:' + o.color + ';width:0%"></div></div>' +
        '<span class="map-num" style="color:' + o.color + '">' + o.num + '/hr</span>' +
        '</div>';
    }).join('');
  }

  const tl = document.getElementById('attack-timeline');
  if(tl){
    const events = [
      {year:'2013', title:'Yahoo Breach', desc:'3 billion accounts compromised — largest breach in history.'},
      {year:'2016', title:'Mirai Botnet', desc:'IoT botnet took down major internet services including Twitter and Netflix.'},
      {year:'2017', title:'WannaCry', desc:'Ransomware infected 200,000+ computers across 150 countries.'},
      {year:'2020', title:'SolarWinds', desc:'Sophisticated supply chain attack compromised US government agencies.'},
      {year:'2021', title:'Colonial Pipeline', desc:'Ransomware attack disrupted US East Coast fuel supply for days.'},
      {year:'2023', title:'MOVEit Breach', desc:'File transfer software flaw exposed data of 2,700+ organizations.'},
      {year:'2024', title:'Record Breaches', desc:'4.1 billion records exposed — the worst year for data breaches on record.'},
    ];
    tl.innerHTML = events.map(function(e){
      return '<div class="timeline-row">' +
        '<div class="tl-year">' + e.year + '</div>' +
        '<div class="tl-dot"></div>' +
        '<div class="tl-content"><div class="tl-title">' + e.title + '</div><div class="tl-desc">' + e.desc + '</div></div>' +
        '</div>';
    }).join('');
  }

  const chart = document.getElementById('attack-chart');
  if(chart){
    const types = [
      {type:'Phishing', pct:85, color:'var(--c-red)'},
      {type:'Malware', pct:68, color:'var(--c-orange)'},
      {type:'Ransomware', pct:52, color:'var(--c-purple)'},
      {type:'Social Eng.', pct:48, color:'var(--gold)'},
      {type:'DDoS', pct:35, color:'var(--teal-bright)'},
      {type:'Insider', pct:22, color:'var(--teal)'},
    ];
    chart.innerHTML = '<div class="chart-title">Attack Types Distribution</div>' +
      types.map(function(t){
        return '<div class="chart-row">' +
          '<span class="chart-label">' + t.type + '</span>' +
          '<div class="chart-bar-wrap"><div class="chart-bar" data-pct="' + t.pct + '" style="background:' + t.color + ';width:0%"></div></div>' +
          '<span class="chart-pct" style="color:' + t.color + '">' + t.pct + '%</span>' +
          '</div>';
      }).join('');
  }
}
...
function animateChartBars(){
  setTimeout(function(){
    document.querySelectorAll('.chart-bar[data-pct], .map-bar[data-pct]').forEach(function(bar){
...
function renderFlashCard(key, questions){
  ...
  let optionsHtml = '<div class="fc-options">' + item.opts.map(function(opt, i){
    return '<div class="fc-opt" data-idx="' + i + '" style="border-color:' + c.border + '">' + opt + '</div>';
  }).join('') + '</div>';
  wrap.innerHTML =
  ...
  document.querySelectorAll('.fc-opt').forEach(function(opt){
    opt.addEventListener('click', function(){
      const idx = parseInt(opt.dataset.idx); const correct = idx === item.a;
